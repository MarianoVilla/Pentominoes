 ...
 *
 */
public class BoxIndexController implements Initializable {
	
	private static final Logger log = LoggerFactory.getLogger(BoxIndexController.class);
	...
	private void setupCboContainer() {
		cboContainers.getItems().setAll(new ClassAItem(1, 0, 0), new ClassBItem(2, 0, 0), new ClassCItem(3, 0, 0));
	...
	private void setupOutputTable() {
		...
		colOutView.setCellFactory(ActionButtonTableCell.<ContainerPackingResult>forTableColumn("View", (ContainerPackingResult a) -> {
		    ...
		    Scene scene = new Scene(root1, 800, 600, true);
	...
	public void addContainer(ActionEvent action) {
		...
		Item toBeAdded = ItemsFactory.Create(cboContainers.getSelectionModel().getSelectedItem().toString(),
	...
	@FXML public void pack()	{
		List<Item> Items = tblContainers.getItems();
		ContainerPackingResult packingResult = BoxPackingService.pack(new DefaultContainer(1), Items);
