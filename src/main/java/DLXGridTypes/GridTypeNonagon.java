...
class GridTypeNonagon
   extends GridType
{
   private static final int[][] tilesInit = {
      {0,8,7,6,5,4,3,2,1}, 
      {3,4,15,14,13,12,11,10,9},
      {4,5,6,22,21,20,19,18,17,16,14,15},
   };
   private static final ICoord[][] adjOffsetInit = {
      {
         new Coord(-1,-1,0,2),new Coord(-1,-1,0,2),new Coord(-1,0,0,1),
         new Coord(0,0,0,2),new Coord(0,0,0,2),new Coord(0,0,0,1),
         new Coord(0,-1,0,2),new Coord(0,-1,0,2),new Coord(-1,-1,0,1),
      },{
         new Coord(0,0,0,-1),new Coord(0,0,0,1),new Coord(0,0,0,1),
         new Coord(1,1,0,-1),new Coord(1,0,0,1),new Coord(1,0,0,1),
         new Coord(1,0,0,-1),new Coord(0,-1,0,1),new Coord(0,-1,0,1),
      },{
         new Coord(0,0,0,-2),new Coord(0,0,0,-2),new Coord(-1,0,0,-1),new Coord(-1,0,0,-1),
         new Coord(0,1,0,-2),new Coord(0,1,0,-2),new Coord(0,1,0,-1),new Coord(0,1,0,-1),
         new Coord(1,1,0,-2), new Coord(1,1,0,-2),new Coord(0,0,0,-1),new Coord(0,0,0,-1),
      },
   };
   private static final int[] tileOrbitInit = {0,0,1};


   public GridTypeNonagon(){
      super("Nonagon", 6,true, false, 23, tilesInit, adjOffsetInit, tileOrbitInit );
   }
   ...
   public void paintIcon(Graphics g, int cx,int cy, int w0, int h0){
      ...
      for(int y=cy-dy; y<cy+h0+dy; y+=dy+dy){
         for(int x=cx-dx; x<cx+w0+dx; x+=dx){
            drawPoly(g,x,y,new int[]{4,5,6,7,8,0,1,2,3});
            drawPoly(g,x,y,new int[]{14,15,4,3,9,10});
            drawPoly(g,x,y,new int[]{11,12,13});
            drawPoly(g,x+dx2,y+dy,new int[]{4,5,6,7,8,0,1,2,3});
            drawPoly(g,x+dx2,y+dy,new int[]{14,15,4,3,9,10});
