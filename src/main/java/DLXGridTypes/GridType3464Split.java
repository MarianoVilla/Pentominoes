   ...
   extends GridType
{
   ...
   private static final ICoord[][] adjOffsetInit = {
      { new Coord(-1,0,0,10), new Coord(0,0,0,1), new Coord(0,0,0,5)},
      { new Coord(0,0,0,5), new Coord(0,0,0,1), new Coord(0,0,0,-1)},
      { new Coord(0,0,0,6), new Coord(0,0,0,1), new Coord(0,0,0,-1)},
      { new Coord(0,0,0,7), new Coord(0,0,0,1), new Coord(0,0,0,-1)},
      { new Coord(0,-1,0,2), new Coord(0,0,0,1), new Coord(0,0,0,-1)},
      { new Coord(-1,-1,0,3), new Coord(0,0,0,-5), new Coord(0,0,0,-1)},
      
      { new Coord(-1,0,0,3), new Coord(0,1,0,-2), new Coord(0,0,0,1), new Coord(0,0,0,-5) },
      { new Coord(0,0,0,-1), new Coord(0,1,0,3), new Coord(0,0,0,1) },
      { new Coord(0,0,0,-1), new Coord(1,1,0,-3), new Coord(0,0,0,1), new Coord(0,0,0,-6) },
      { new Coord(0,0,0,-1), new Coord(1,0,0,-3), new Coord(0,0,0,1) },
      { new Coord(0,0,0,-1), new Coord(1,0,0,-10), new Coord(0,-1,0,-3), new Coord(0,0,0,-7) },
   };
   private static int[] tileOrbitInit = {0,0,0,0,0,0,1,2,1,2,1};
   private int blockHeight=0;
   private static double factor=Math.sqrt(3);

   public GridType3464Split(){
      super("3464Split","Split Rhombitrihex",6,true, false, 13, tilesInit, adjOffsetInit, tileOrbitInit);
   }
   ...
   public void paintIcon(Graphics g, int cx, int cy, int w, int h){
      for(int y=cy-icontileheight; y<cy+h+icontileheight; y+=icontileheight*2+icontilesize*2){
         for(int x=cx-icontilesize/2; x<cx+w; x+=icontilesize*6+2*icontileheight){
            ...
            g.drawLine(x,y,x+2*icontilesize,y+2*icontileheight);
            g.drawLine(x-icontilesize,y+icontileheight,x+3*icontilesize,y+icontileheight);
            g.drawLine(x,y+2*icontileheight,x+2*icontilesize,y);
            ...
            g.drawLine(x+3*icontilesize+icontileheight,y+icontilesize+icontileheight,x+5*icontilesize+icontileheight,y+icontilesize+3*icontileheight);
            g.drawLine(x+2*icontilesize+icontileheight,y+icontilesize+2*icontileheight,x+6*icontilesize+icontileheight,y+icontilesize+2*icontileheight);
            g.drawLine(x+3*icontilesize+icontileheight,y+icontilesize+3*icontileheight,x+5*icontilesize+icontileheight,y+icontilesize+icontileheight);
   ...
   protected void recalcVertices(){
      ...
      vertexx[7] = -blockHeight-blockSize;
      vertexy[7] = blockHeight+blockSize;
      vertexx[8] = -blockHeight;
      vertexy[8] = blockHeight*2+blockSize;
      vertexx[9] = 0;
      vertexy[9] = blockHeight*2+blockSize*2;
      vertexx[10] = blockSize*2;
      vertexy[10] = blockHeight*2+blockSize*2;
      vertexx[11] = blockSize*2+blockHeight;
      vertexy[11] = blockHeight*2+blockSize;
      vertexx[12] = blockSize*3+blockHeight;
      vertexy[12] = blockHeight+blockSize;
   }
