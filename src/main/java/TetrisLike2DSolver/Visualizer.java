...
public class Visualizer extends javax.swing.JFrame {
    ...
    private Pentominos2DSolverMT smt;
    ...
    public Visualizer(final Object solver, JFrame caller) {
        ...
        if (solver instanceof Pentominos2DSolverMT) {
            smt = (Pentominos2DSolverMT) solver;
            nPieces = smt.getNumberOfPieces();
            pentominoes = smt.getDistinctPentominoes();
            width = smt.getWidth();
        ...
        if (sol != null) {
            for (int y = 0; y < height; y++) {
                for (int x = 0; x < width; x++) {
                    if (sol[y][x] >= 0) {
                    	float S = sol[y][x];
                    	Pentomino pento = pentominoes.stream().filter((p) -> p.getTypeID() == S).findFirst().orElse(new NullPentomino(0,0,0));
                    	
                        g.setColor(pento.getColor().getAwtColor());
