...
public class Pentominoes2DSolverMT {
	...
	private boolean solved = false; // set to indicate that the solve() method has been called and has finished
									// processing (or was canceled)
	private Solution cachedResult; // the result of the solve() method so it doesn't need to be calculated again.
									// null if no solution was found (or solve() was canceled)
	...
	private static class BoardState {
		...
		private int p = 1; // pointer to next block to add (starts from 1, ends at blocks.length). this
							// means that the higher p is, the closer we are to completing the puzzle
		...
		/**
		 * clones a work unit. p is incremented because work units are cloned only when
		 * the next block needs to be added
		 *
	...
	public static class Solution {
		 ...
		 * @param s work unit
		 */
		private Solution(BoardState s, ArrayList<Pentomino> Pentominoes) {
			board = new int[s.board.length][s.board[0].length];
			for (int y = 0; y < board.length; y++) {
				System.arraycopy(s.board[y], 0, board[y], 0, board[0].length);
			}
		...
		/**
		 * creates a new instance using the given reference. used by save/load methods
		 * to speed things up
		 *
	...
	private LinkedList<BoardState> queue = new LinkedList<>(); // this contains all the work units. work units are added
																// and removed by SlaveThreads
	private long iterations = 0; // number of processed work units

	private boolean stopASAP = false; // set to true to cancel computation
	private boolean paused = false, masterPaused = true; // used by save/load methods. setting paused to true causes all
															// threads to stop as soon as possible in a safe state.
															// masterPaused is set to true by the master thread when
															// it's paused in a safe state

	private class SlaveThread extends Thread {

		private boolean waiting = false; // set to true when no work units are available. if all threads are waiting and
											// the work queue is empty, the puzzle has no solution and the master thread
											// will stop processing
		private Solution solution; // set to an instance of Solution if this threads finds a solution
		private int[][] __board; // board that it's currently working on. a reference is kept here so a copy can
									// be passed to the gui to show the animation. ignore.
		...
		public void run() {
			...
			for (;;) {
				...
				while (true) {
					...
					synchronized (queue) {
						if (queue.size() > 0) {
							// a work unit is available. fetch and remove from queue (actually a stack so
							// work units with higher p (closer to solution) are processed first)
							waiting = false;
				...
				// This way, each block has a unique ID. It's useful to separate them, but we
				// might want to store a block type ID in the Item itself.
				final int p = workunit.p;
				// final char block = blocks[p - 1]; //which block do we have to place? (p-1
				// because p starts from 1)
				final Pentomino pentomino = pentominoes.get(p - 1);
				final int pentoID = Pentomino.typeIDMap.get(pentomino.getTypeChar());
				LinkedList<BoardState> toAdd = new LinkedList<>(); // this list will contain all the newly created work
																	// units
				...
				if (pentomino instanceof TPentomino) {
					...
					for (int y = 0; y <= board.length - 3; y++) {
						for (int x = 0; x <= board[0].length - 3; x++) {
							...
							if (board[y][x] == 0 && board[y][x + 1] == 0 && board[y + 1][x + 1] == 0
								...
								board[y][x] = pentoID;
								board[y][x + 1] = pentoID;
								board[y + 1][x + 1] = pentoID;
								board[y + 2][x + 1] = pentoID;
								board[y][x + 2] = pentoID;
								if (p == blocks.length) { // solution found!
									solution = new Solution(workunit, pentominoes);
									slavePaused = true;
									return;
								} else { // needs more work. add to new state to work queue, but only if it's not a
											// stupid config
					...
					for (int y = 0; y <= board.length - 3; y++) {
						for (int x = 0; x <= board[0].length - 3; x++) {
							if (board[y][x] == 0 && board[y + 1][x] == 0 && board[y + 1][x + 1] == 0
									&& board[y + 1][x + 2] == 0 && board[y + 2][x] == 0) {
								// we found a hole that fits this block, we'll place it here and see if the
								// puzzle can be solved
								board[y][x] = pentoID;
								board[y + 1][x] = pentoID;
								board[y + 1][x + 1] = pentoID;
								board[y + 1][x + 2] = pentoID;
								board[y + 2][x] = pentoID;
								if (p == blocks.length) { // solution found!
									solution = new Solution(workunit, pentominoes);
									slavePaused = true;
					...
					for (int y = 0; y <= board.length - 3; y++) {
						for (int x = 0; x <= board[0].length - 3; x++) {
							if (board[y + 2][x] == 0 && board[y][x + 1] == 0 && board[y + 1][x + 1] == 0
									&& board[y + 2][x + 1] == 0 && board[y + 2][x + 2] == 0) {

								board[y + 2][x] = pentoID;
								board[y][x + 1] = pentoID;
								board[y + 1][x + 1] = pentoID;
								board[y + 2][x + 1] = pentoID;
								board[y + 2][x + 2] = pentoID;
								if (p == blocks.length) { // solution found!
									solution = new Solution(workunit, pentominoes);
									slavePaused = true;
									return;
					...
					for (int y = 0; y <= board.length - 3; y++) {
						for (int x = 0; x <= board[0].length - 3; x++) {
							if (board[y][x + 2] == 0 && board[y + 1][x] == 0 && board[y + 1][x + 1] == 0
									&& board[y + 1][x + 2] == 0 && board[y + 2][x + 2] == 0) {
								// we found a hole that fits this block, we'll place it here and see if the
								// puzzle can be solved
								board[y][x + 2] = pentoID;
								board[y + 1][x] = pentoID;
				...
				// P shaped.
					...
					for (int y = 0; y <= board.length - 2; y++) {
						for (int x = 0; x <= board[0].length - 3; x++) {
							if (board[y + 1][x] == 0 && board[y][x + 1] == 0 && board[y][x + 2] == 0
									&& board[y + 1][x + 1] == 0 && board[y + 1][x + 2] == 0) {
								board[y + 1][x] = pentoID;
								board[y][x + 1] = pentoID;
								board[y][x + 2] = pentoID;
				...
				// L shaped.
					...
					for (int y = 0; y <= board.length - 2; y++) {
						for (int x = 0; x <= board[0].length - 4; x++) {
							if (board[y + 1][x] == 0 && board[y + 1][x + 1] == 0 && board[y + 1][x + 2] == 0
									&& board[y + 1][x + 3] == 0 && board[y][x + 3] == 0) {
								// we found a hole that fits this block, we'll place it here and see if the
								// puzzle can be solved
								board[y + 1][x] = pentoID;
								board[y + 1][x + 1] = pentoID;
								board[y + 1][x + 2] = pentoID;
								board[y + 1][x + 3] = pentoID;
								board[y + 1][x + 3] = pentoID;
								if (p == blocks.length) { // solution found!
									solution = new Solution(workunit, pentominoes);
									slavePaused = true;
					...
					for (int y = 0; y <= board.length - 4; y++) {
						for (int x = 0; x <= board[0].length - 2; x++) {
							if (board[y][x] == 0 && board[y][x + 1] == 0 && board[y + 1][x + 1] == 0
									&& board[y + 2][x + 1] == 0 && board[y + 3][x + 1] == 0) {
								// we found a hole that fits this block, we'll place it here and see if the
								// puzzle can be solved
								board[y][x] = pentoID;
								board[y][x + 1] = pentoID;
					...
					for (int y = 0; y <= board.length - 2; y++) {
						for (int x = 0; x <= board[0].length - 4; x++) {
							if (board[y][x] == 0 && board[y + 1][x] == 0 && board[y][x + 1] == 0 && board[y][x + 2] == 0
									&& board[y][x + 3] == 0) {
								// we found a hole that fits this block, we'll place it here and see if the
								// puzzle can be solved
								board[y][x] = pentoID;
					...
					for (int y = 0; y <= board.length - 4; y++) {
						for (int x = 0; x <= board[0].length - 2; x++) {
							if (board[y][x + 1] == 0 && board[y + 1][x + 1] == 0 && board[y + 2][x + 1] == 0
									&& board[y + 3][x + 1] == 0 && board[y + 3][x] == 0) {
								// we found a hole that fits this block, we'll place it here and see if the
								// puzzle can be solved
								board[y][x + 1] = pentoID;
								board[y + 1][x + 1] = pentoID;
					...
					for (int y = 0; y <= board.length - 2; y++) {
						for (int x = 0; x <= board[0].length - 4; x++) {
							if (board[y][x] == 0 && board[y + 1][x] == 0 && board[y + 1][x + 1] == 0
									&& board[y + 1][x + 2] == 0 && board[y + 1][x + 3] == 0) {
								// we found a hole that fits this block, we'll place it here and see if the
								// puzzle can be solved
								board[y][x] = pentoID;
								board[y + 1][x] = pentoID;
					...
					for (int y = 0; y <= board.length - 4; y++) {
						for (int x = 0; x <= board[0].length - 2; x++) {
							if (board[y][x] == 0 && board[y][x + 1] == 0 && board[y + 1][x] == 0 && board[y + 2][x] == 0
									&& board[y + 3][x] == 0) {
								// we found a hole that fits this block, we'll place it here and see if the
								// puzzle can be solved
								board[y][x] = pentoID;
	...
	private SlaveThread[] slaves = new SlaveThread[Runtime.getRuntime().availableProcessors()]; // 1 slave thread per
																								// core (+1 master
																								// thread)
	...
	public Pentominoes2DSolverMT(int width, int height, int lBlocks, int pBlocks, int tBlocks) {
		...
		for (int i = 0; i < lBlocks; i++) {
			blocks[p] = L;
			pentominoes.add(p++, new LPentomino(i, 0.5, 1.0));
		}
		for (int i = 0; i < tBlocks; i++) {
			blocks[p] = T;
			pentominoes.add(p++, new TPentomino(i, 0.5, 2.0));
		}
		for (int i = 0; i < pBlocks; i++) {
			blocks[p] = P;
			pentominoes.add(p++, new PPentomino(i, 0.5, 3.0));
		}
		/*
		 * if((width * height) % 5 != 0) { solved = true; cachedResult = null; return; }
		 */
	...
	public Pentominoes2DSolverMT(int width, int height, List<Pentomino> pentos) {
		...
		w = width;
		h = height;
		int nBlocks = pentos.size();
		blocks = new char[nBlocks];
		int p = 0;

		for (Pentomino pento : pentos) {
			if (pento instanceof LPentomino) {
				blocks[p] = L;
				pentominoes.add(p++, pento);
			}
			if (pento instanceof PPentomino) {
				blocks[p] = T;
				pentominoes.add(p++, pento);
			}
			if (pento instanceof TPentomino) {
				blocks[p] = P;
				pentominoes.add(p++, pento);
			}
		}
	...
	public synchronized Solution solve() {
		...
		for (;;) {
			...
			// check if all slaves are waiting and the queue is empty (puzzle has no
			// solution)
			synchronized (queue) {
				int qLen = queue.size();
				if (qLen == 0) {
	...
	/**
	 * used by gui to fetch the current board status from all slaves (copy, or null
	 * if concurrent access occurs). ignore.
	 *
	 * @return array of nCores int[rows][cols]. Only 1 if puzzle is solved. some may
	 *         be NULL. NULL if puzzle has no solution.
	 */
	public int[][][] __status() {
		if (solved) {
	...
	/**
	 * cancels processing. use to stop solve() method while processing. waits for
	 * threads to actually stop.
	 */
	public void cancel() {
		stopASAP = true;
		while (!masterPaused) {
	 ...
	 *
	 * java serialization is not used because it produced huuuuuuge files, so here's
	 * the file format instead:
	 *
	 ...
	 * unicode)<br>
	 * 1 byte: solved status: 2=solved, has no solution, 1=solved, solution follows,
	 * 0=not solved yet, saved state follows<br>
	 * *** if 1: 4*rows*cols bytes contain the solution board as a sequence of ints,
	 * serialized "c style". see serializeBoard and deserializeBoard for details<br>
	 * *** if 0: 4 bytes contain the number of work units that follow (as int). for
	 * each unit: 4*rows*cols bytes for the board (see row above) + 4 bytes for the
	 * value of p for that unit (as int)<br>
	 * 4 bytes: magic number again, indicating proper end of file
	 */
	private static final byte[] MAGIC = new byte[] { 0x3e, 0x6d, 0x75, 0x68 };

	/**
	 * write board to specified DataOutputStream. it dumps the contents "c style"
	 * (row per row, element by element for each row). size is not written!
	 *
	...
	/**
	 * deserialize a bunch of ints from the given DataInputStream into a board, and
	 * stores it into the given board. rows and cols are taken from board size.
	 *
	 * @param board target
	 * @param is    stream
		...
		synchronized (saveLock) {
			try {
				...
				if (solved) { // already solved
					...
					// not solved yet
					// check to see if the puzzle is actually solved but the master hasn't noticed
					// yet
					if (queue.isEmpty()) {
						// puzzle is impossible
						dos.writeByte(2);
	...
	/**
	 * load state from specified InputStream. solve() needs to be called to resume
	 * computation
	 *
	 * @param is stream
	...
	public static Pentominoes2DSolverMT loadState(InputStream is) throws IOException {
		...
		for (int i = 0; i < nBlocks; i++) {
			...
			if (blocks[i] != T && blocks[i] != L && blocks[i] != P) {
				throw new IOException("Not a valid saved state");
			}
		}
	...
	/*	    *//**
				 ...
				 */
	/*
	 * public int getTBlocks() { int ret = 0; for (char c : blocks) { if (c == T) {
	 * ret++; } } return ret; }
	 * 
	 *//**
		 *
		 * @return number of P blocks
		 */
	/*
	 * public int getPBlocks() { int ret = 0; for (char c : blocks) { if (c == P) {
	 * ret++; } } return ret; }
	 * 
	 * 
	 *//**
		 ...
		 *//*
			 * public int getLBlocks() { int ret = 0; for (char c : blocks) { if (c == L) {
			 * ret++; } } return ret; }
			 */
	...
	/**
	 *
	 * @return true if the puzzle has been solved (even if it has no solution) (in
	 *         other words, if the solve() method was called and has finished or was
	 *         canceled)
	 */
	...
	public ArrayList<Pentomino> getDistinctPentominoes() {
		Set<Character> set = new HashSet<>(pentominoes.size());
		return (ArrayList<Pentomino>) this.pentominoes.stream().filter(p -> set.add(p.getTypeChar()))
